...
app.get("/records", (req, res) => {
  ...
  const result = data.slice(skip, end);
...
app.get("/", (req, res, next) => {
  //  const record = data.find(c => c.id === parseInt(req.params.id));

  // if (!record) {
  //     res.status(404).send('provide correct id');
  // }
  // res.send(record);
  ...
  const filteredUsers = data.filter((records) => {
    let isValid = true;
    for (key in filters) {
      //  console.log(key, records[key], filters[key]);
      isValid = isValid && records[key] == filters[key];
    }
    return isValid;
...
//   const skip = (page - 1) * limit;
//   const end = page * limit;
var count = Object.keys(filteredUsers).length
console.log(count);
